...
let burger_catalog_img_buttom = document.getElementById('burger_catalog_img_buttom_id'); // (адаптированная иконка каталога каталога);
...
burger_catalog_img_buttom.addEventListener('click', _=>{
    open_close_menu_f();
    position_menu_f();
    adjustment_width_hidden_catalog_f();
});
...
function adjustment_width_hidden_catalog_f() {
    ...
    if (width_windows >= 500) {
        burger_catalog_open.style.width = `${(width_windows / 4) * 1.515}px`; // (ширина скрытого каталога бургер);
    }
    else {
        burger_catalog_open.style.width = `${(width_windows / 4.25) * 1.515}px`; // (ширина скрытого каталога бургер);
    }
}
